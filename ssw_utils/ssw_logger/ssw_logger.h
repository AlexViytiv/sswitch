...
#include "../ssw_common/ssw_status.h"
...
#define LOG_CLI(format, ...) printf(" [%s] %s:%-4d %-20s " format, logger_get_time_str(), __FILENAME__, __LINE__, __FUNCTION__, ## __VA_ARGS__)
 ...
 */
ssw_status_t logger_set_glog_level(log_level_t new_level);
 ...
 * \return status_code_t of operation result.
 */
ssw_status_t logger_get_glog_level(log_level_t * existing_level);
